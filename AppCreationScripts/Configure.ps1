...
#> 
   ...
   $configFile = $pwd.Path + "\..\MiddletierAPI\config.json"
   Write-Host "Updating the sample code ($configFile)"
   $dictionary = @{ "Web_Api_Scope" = ("api://"+$DownstreamAPIAadApplication.AppId+"/access_as_user") };
   ReplaceInTextFile -configFilePath $configFile -dictionary $dictionary

   # Update config file for 'spa'
   $configFile = $pwd.Path + "\..\SPA\src\authConfig.js"
   Write-Host "Updating the sample code ($configFile)"
   $dictionary = @{ "Application_Id" = $spaAadApplication.AppId;"https://login.microsoftonline.com/Tenant_Id" = "https://login.microsoftonline.com/"+$tenantId;"Web_Api_Scope" = ("api://"+$MiddletierAPIAadApplication.AppId+"/access_as_user") };
   ReplaceInTextFile -configFilePath $configFile -dictionary $dictionary
   Write-Host ""
